...
public class DocumentSearchListResourceEndpointTest {
  ...
  public void shouldReturn200ForEmptySearch() throws JsonMappingException, JsonProcessingException {
    ...
    final List<Concept> searchConceptsResults =
        createClonedConceptsListFromContentList(contentLists);
    searchConceptsResults.forEach(concept -> concept.setOriginalUUID(concept.getUuid().toString()));
    final Set<String> searchConceptsUUIDs =
        searchConceptsResults.stream()
    ...
    searchConceptsParam.forEach(
        searchConceptParam -> assertTrue(searchConceptsUUIDs.contains(searchConceptParam)));
    assertThat(searchConceptsParam.size(), equalTo(searchConceptsResults.size()));

    assertThat("response", clientResponse, hasProperty("status", equalTo(200)));
    final List<ContentList> retrievedLists =
        Arrays.asList(clientResponse.readEntity(ContentList[].class));
    retrievedLists.forEach(
        list -> assertTrue(searchConceptsUUIDs.contains(list.getConcept().getUuid().toString())));

    assertThat(retrievedLists.size(), equalTo(contentLists.size()));
  }

  @Test
  public void shouldReturn200ForGivenConceptNoConcordance()
    ...
    final Response clientResponse =
        resources
            .client()
            .target("/search/lists")
            .queryParam("conceptUUID", conceptParams[0].toString())
            .request()
            .get();
  ...
  public void shouldReturn200ForGivenConceptWithConcordancesAndListTypeAndSearchTerm()
    ...
    final Identifier identifier =
        new Identifier("http://api.ft.com/system/UPP", UUID.randomUUID().toString());
    ...
    final Set<String> searchConceptsUUIDs =
        contentLists.stream()
            .map(c -> c.getConcept().getUuid().toString())
            .collect(Collectors.toSet());
    ...
    final UUID[] conceptParams =
        concordances.stream()
            .map(c -> UUID.fromString(c.getIdentifier().getIdentifierValue()))
            .toArray(UUID[]::new);
    ...
    final Response clientResponse =
        resources
            .client()
            .target("/search/lists")
            .queryParam("conceptUUID", conceptUUID)
  ...
  @Test
  public void shouldReturn400ForInvalidConceptUUID()
      throws JsonMappingException, JsonProcessingException {
  ...
  private static Map<Pair<String, Operation>, HandlerChain> getCollectionMap() {
    final Handler conceptUuidValidationHandler =
        new ConceptUuidValidationHandler(new UuidValidator());
    final Handler getConcordedConceptsHandler =
    ...
    final Target applyConcordedConceptsToLists =
  ...
  private static ContentList createContentList(
      final String listUuid,
      final String firstContentUuid,
      final String secondContentUuid,
      final boolean addConcept) {
    ...
    final ContentList.Builder builder =
        ...
        .map(
            contentList -> {
              final Document document =
                  new Document(objectMapper.convertValue(contentList, Map.class));
              document.put(
                  "concept",
                  new Document(objectMapper.convertValue(contentList.getConcept(), Map.class)));
              return document;
            })
        .collect(Collectors.toList());
  }

  private List<Concept> createClonedConceptsListFromContentList(
      final List<ContentList> contentLists) throws JsonProcessingException {
    return contentLists.stream()
        .map(
            contentList ->
                assertDoesNotThrow(
                    () ->
                        objectMapper.readValue(
                            objectMapper.writeValueAsString(contentList.getConcept()),
                            Concept.class)))
        .collect(Collectors.toList());
  }
}
