...
// private static ContentListValidator contentListValidator =
...
// content.put("publishedDate", new
// SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").format(lastPublicationDate));
...
// Handler multipleUuidValidationHandler = new
// MultipleUuidValidationHandler(uuidValidator);
// Handler extractUuidsHandlers = new ExtractUuidsHandler();
// Handler preSaveFieldRemovalHandler = new PreSaveFieldRemovalHandler();
// Target findResourceByUuid = new
// FindResourceByUuidTarget(documentStoreService);
// Target findMultipleResourcesByUuidsTarget = new
// FindMultipleResourcesByUuidsTarget(documentStoreService);
...
// new HandlerChain().addHandlers(extractUuidsHandlers,
// multipleUuidValidationHandler).setTarget(findMultipleResourcesByUuidsTarget));
// collections.put(new Pair<>("content", Operation.GET_MULTIPLE_FILTERED),
// new
// HandlerChain().addHandlers(multipleUuidValidationHandler).setTarget(findMultipleResourcesByUuidsTarget));
// collections.put(new Pair<>("content", Operation.GET_BY_ID),
// new
// HandlerChain().addHandlers(uuidValidationHandler).setTarget(findResourceByUuid));
// collections.put(new Pair<>("content", Operation.ADD),
// new HandlerChain().addHandlers(uuidValidationHandler,
// preSaveFieldRemovalHandler).setTarget(writeDocument));
...
// Object.class))).thenReturn(DocumentWritten.created(document));
// }
...
// when(documentStoreService.write(eq(RESOURCE_TYPE), anyMapOf(String.class,
// Object.class))).thenReturn(DocumentWritten.updated(document));
...
// when(documentStoreService.write(eq(RESOURCE_TYPE), any())).thenThrow(new
// ExternalSystemUnavailableException("Cannot connect to Mongo"));
...
// when(documentStoreService.write(eq(RESOURCE_TYPE),
// contentCaptor.capture())).thenReturn(DocumentWritten.created(localDoc));
...
// doThrow(new
// DocumentNotFoundException(UUID.fromString(uuid))).when(documentStoreService).delete(eq(RESOURCE_TYPE),
// any(UUID.class));
...
// Mongo")).when(documentStoreService).delete(eq(RESOURCE_TYPE),
// any(UUID.class));
...
// any(UUID.class))).thenReturn(document);
...
// when(documentStoreService.findByUuids(eq(RESOURCE_TYPE),
// eq(uuids))).thenReturn(documents);
...
// @SuppressWarnings("unchecked") final List<Map<String, Object>> actual =
// clientResponse.readEntity(List.class);
// assertThat("documents", actual.size(), equalTo(2));

// List<String> actualIds = actual.stream().map(m -> (String)
// m.get("uuid")).collect(Collectors.toList());
...
// when(documentStoreService.findByUuids(eq(RESOURCE_TYPE),
// eq(uuids))).thenReturn(Collections.singleton(document2));
...
// assertThat("document list", actualIds,
// equalTo(Collections.singletonList(id2)));
// }
...
// String id1 = uuid1.toString();
// when(documentStoreService.findByUuid(eq(RESOURCE_TYPE),
// any(UUID.class))).thenThrow(new DocumentNotFoundException(null));
...
// @SuppressWarnings("rawtypes") final List actual =
// clientResponse.readEntity(List.class);
...
// when(documentStoreService.findByUuids(eq(RESOURCE_TYPE),
// eq(uuidSet))).thenReturn(documents);
...
// when(documentStoreService.findByUuids(eq(RESOURCE_TYPE),
// eq(uuidSet))).thenReturn(Collections.singleton(document));
...
// doThrow(new ExternalSystemUnavailableException("Cannot connect to
// Mongo")).when(documentStoreService).findByUuid(eq(RESOURCE_TYPE),
// any(UUID.class));
...
// private void validateErrorMessage(String expectedErrorMessage, Response
// clientResponse) {
