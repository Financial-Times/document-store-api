...
// private static final String IDENTIFIER_VALUE =
// "http://www.example.org/here-is-the-news";
// private static final Document METHODE_AUTHORITY = new
// Document().append("authority", "http://api.ft.com/system/FTCOM-METHODE");
// private static final Document WORDPRESS_AUTHORITY = new
// Document().append("authority", "http://api.ft.com/system/FT-LABS-WP-1-335");
...
// content = ImmutableMap.<String, Object>builder()
...
// outboundContent = ImmutableMap.<String, Object>builder()
// .put("uuid", uuid.toString())
// .put("title", "Here is the news")
// .put("byline", "By Bob Woodward")
// .put("bodyXML", "xmlBody")
...
// Set<Map<String, Object>> content =
// mongoDocumentStoreService.findByUuids("content", uuids);

// assertThat(content.size(), equalTo(2));

// List<String> actualUuids = content.stream().map(m -> (String)
// m.get("uuid")).collect(Collectors.toList());
// assertThat(actualUuids, contains(uuid.toString(), uuid2.toString()));

// assertThat(content.stream().anyMatch(m -> m.containsKey("_id")),
// equalTo(false));
// }
...
// assertThat((String) content.iterator().next().get("uuid"),
// equalTo(uuid.toString()));
// }
...
// Document foundContent = collection.find().filter(Filters.eq("uuid",
// uuid.toString())).first();
...
// assertThat((Date) foundContent.get("publishedDate"),
// is(lastPublicationDate));
...
// assertThat(collection.find().filter(Filters.eq("uuid",
// uuid.toString())).first(), nullValue());
// }
...
// String expectedMessage = String.format("Document with uuid : %s not found!",
// uuid);
...
// mongoDocumentStoreService.findByIdentifier(DB_COLLECTION, AUTHORITY,
// IDENTIFIER_VALUE);
...
// assertThat("UUID index", indexes.get().anyMatch(doc ->
// uuidKey.equals(doc.get("key")) && doc.getBoolean("unique")), is(true));
...
// assertThat("Identifiers index", indexes.get().anyMatch(doc ->
// identifierKey.equals(doc.get("key"))), is(true));
// }
...
// assertThat(actual.toString(), equalTo("{ \"uuid\" :
// \"d08ef814-f295-11e6-a94b-0e7d0412f5a5\" }\n" +
// "{ \"uuid\" : \"8ae3f1dc-f288-11e6-8758-6876151821a6\" }\n"));
// }

// @Test
// public void idsAndSourceOfForContentShouldBeRetrievedSuccessfully() throws
// IOException {
// final String firstUUID = "d08ef814-f295-11e6-a94b-0e7d0412f5a5";
// final Document firstDocument = new Document()
// .append("uuid", firstUUID)
...
// assertThat(actual.toString(), equalTo("{ \"uuid\" :
// \"d08ef814-f295-11e6-a94b-0e7d0412f5a5\", \"identifiers\" : { \"authority\" :
// \"http://api.ft.com/system/FT-LABS-WP-1-335\" } }\n" +
// "{ \"uuid\" : \"8ae3f1dc-f288-11e6-8758-6876151821a6\", \"identifiers\" : {
