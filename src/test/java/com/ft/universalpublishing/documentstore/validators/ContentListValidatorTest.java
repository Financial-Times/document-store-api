...
public class ContentListValidatorTest {
  ...
  public void shouldFailValidationIfContentListIsNull() {
    Exception exception =
        assertThrows(ValidationException.class, () -> contentListValidator.validate(uuid, null));
  ...
  public void shouldFailValidationIfUuidOnContentDoesNotMatchUuid() {
    ...
    String expectedMessage =
        String.format(
            "uuid in path %s is not equal to uuid in submitted list %s", mismatchedUuid, uuid);
    assertThat(exception.getMessage(), equalTo(expectedMessage));
  }
}
