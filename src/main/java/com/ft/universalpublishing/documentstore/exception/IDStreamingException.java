...
public class IDStreamingException extends WebApplicationException {
    ...
    public IDStreamingException(String collection) {
        super(status(Response.Status.INTERNAL_SERVER_ERROR).type(APPLICATION_JSON).entity(
                singletonMap("message", "An error occurred when trying to get ids for collection " + collection))
                .build());
        response = constructResponse(collection);

        FluentLoggingBuilder.getNewInstance(this.getClass().getCanonicalName(), "IDStreamingException")
    ...
    private Response constructResponse(String collection) {
        response = status(Response.Status.INTERNAL_SERVER_ERROR).type(APPLICATION_JSON).entity(
                singletonMap("message", "An error occurred when trying to get ids for collection " + collection))
                .build();
        return response;
    }
