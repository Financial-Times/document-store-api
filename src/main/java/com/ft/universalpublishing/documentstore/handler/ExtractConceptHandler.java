...
public class ExtractConceptHandler implements Handler {
  ...
  private static final Pattern LIST_QUERY_PARAM_PATTERN =
      Pattern.compile(LIST_QUERY_PARAM_TEMPLATE);
  ...
  public void handle(Context context) {
    ...
    if (queryParameters.isEmpty()) {
      throw ClientError.status(SC_BAD_REQUEST)
          .error("Expected at least one query parameter")
          .exception();
    }
    ...
    for (String key : keys) {
      ...
      if (found) {
        ...
        try {
          ...
          throw ClientError.status(SC_BAD_REQUEST)
              .error("The concept ID is not a valid UUID")
              .exception();
        }
      }
    }
