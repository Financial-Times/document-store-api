...
public class ContentListValidator {
  ...
  public void validate(String uuidString, ContentList contentList) {
    ...
    if (!uuidString.equals(contentList.getUuid())) {
      String message =
          String.format(
              "uuid in path %s is not equal to uuid in submitted list %s",
              uuidString, contentList.getUuid());
    ...
    if (concept != null) {
      if (concept.getUuid() == null) {
        throw new ValidationException(
            "if a concept is supplied it must have a non-empty uuid field");
      }
      if (Strings.isNullOrEmpty(concept.getPrefLabel())) {
        throw new ValidationException(
            "if a concept is supplied it must have a non-empty prefLabel field");
      }
    }
    ...
    for (ListItem item : items) {
      if ((item.getUuid() == null || item.getUuid().isEmpty())
        ...
        throw new ValidationException(
            "list items must have a non-empty uuid or a non-empty webUrl");
      }
