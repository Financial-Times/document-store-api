...
public class RewriteContentElementEventHandler extends BaseXMLEventHandler {

	private static final String TYPE_ATTRIBUTE_NAME = "type";
	private static final String ID_ATTRIBUTE_NAME = "id";
	private static final String URL_ATTRIBUTE_NAME = "url";
	private static final String REWRITE_ELEMENT = "ft-content";
	private static final String TYPE_ATTR_MISSING_MESSAGE = "Type attribute is missing from a content element";
	...
	public ContentBodyProcessingContext getContext() {
		return this.context;
	}
	...
	public void handleStartElementEvent(final StartElement event, final XMLEventReader xmlEventReader, final BodyWriter eventWriter,
        ...
        try {
			...
			eventWriter.writeStartTag(REWRITE_ELEMENT, rewrittenAttributes);
	...
	public void handleEndElementEvent(final EndElement event, final XMLEventReader xmlEventReader, final BodyWriter eventWriter) throws XMLStreamException {
		eventWriter.writeEndTag(REWRITE_ELEMENT);
		context.setProcessingContent(false);
	}

	private String getAttribute(final StartElement event, final String name, final String message) {
		final Attribute typeAttribute = event.getAttributeByName(new QName(name));
		if (typeAttribute == null) {
			throw new BodyTransformationException(message);
		}

		final String type = typeAttribute.getValue();
